...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_first_env_non_empty(&["ANTHROPIC_API_KEY", "POE_API_KEY"])?;
        let auth_token = read_first_env_non_empty(&["ANTHROPIC_AUTH_TOKEN", "POE_API_KEY"])?;
...
impl ClawApiClient {
    ...
    async fn send_raw_request(
        ...
        if std::env::var("CLAW_DEBUG_REQUESTS").ok().as_deref() == Some("1") {
            if let Ok(serialized) = serde_json::to_string_pretty(request) {
                eprintln!("--- CLAW DEBUG REQUEST {request_url} ---");
                eprintln!("{serialized}");
                eprintln!("--- END CLAW DEBUG REQUEST ---");
            }
        }
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        if let Some(api_key) = read_first_env_non_empty(&["ANTHROPIC_API_KEY", "POE_API_KEY"])? {
            return match read_first_env_non_empty(&["ANTHROPIC_AUTH_TOKEN", "POE_API_KEY"])? {
        ...
        if let Some(bearer_token) =
            read_first_env_non_empty(&["ANTHROPIC_AUTH_TOKEN", "POE_API_KEY"])?
        {
...
pub fn has_auth_from_env_or_saved() -> Result<bool, ApiError> {
    Ok(read_first_env_non_empty(&["ANTHROPIC_API_KEY", "POE_API_KEY"])?.is_some()
        || read_first_env_non_empty(&["ANTHROPIC_AUTH_TOKEN", "POE_API_KEY"])?.is_some()
...
pub fn read_base_url() -> String {
    std::env::var("ANTHROPIC_BASE_URL")
        .or_else(|_| std::env::var("POE_BASE_URL"))
...
mod tests {
    ...
    #[test]
    fn auth_source_from_env_accepts_poe_api_key_alias() {
        let _guard = env_lock();
        std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
        std::env::remove_var("ANTHROPIC_API_KEY");
        std::env::set_var("POE_API_KEY", "poe-test-key");
        let auth = AuthSource::from_env().expect("poe env auth");
        assert_eq!(auth.api_key(), Some("poe-test-key"));
        assert_eq!(auth.bearer_token(), Some("poe-test-key"));
        std::env::remove_var("POE_API_KEY");
    }

    #[test]
    fn read_base_url_accepts_poe_alias() {
        let _guard = env_lock();
        std::env::remove_var("ANTHROPIC_BASE_URL");
        std::env::set_var("POE_BASE_URL", "https://api.poe.com");
        assert_eq!(super::read_base_url(), "https://api.poe.com");
