...
use api::{
    detect_provider_kind, resolve_startup_auth_source, AuthSource, ClawApiClient, ContentBlockDelta,
    InputContentBlock, InputMessage, MessageRequest, MessageResponse, OutputContentBlock,
    ProviderClient as ModelProviderClient, ProviderKind,
...
fn summarize_permission_input(request: &runtime::PermissionRequest) -> String {
    let parsed: serde_json::Value = serde_json::from_str(&request.input)
    ...
    match request.tool_name.as_str() {
        "bash" | "Bash" => parsed
            ...
            .unwrap_or_else(|| format!("  Input        {}", request.input)),
        _ => {
            let summary = summarize_tool_payload(&request.input);
            if summary.is_empty() {
                format!("  Input        {}", truncate_for_summary(&request.input, 180))
            } else {
...
impl DefaultRuntimeClient {
    fn new(
        ...
        let default_auth = match detect_provider_kind(&model) {
            ProviderKind::ClawApi => Some(resolve_cli_auth_source()?),
            ProviderKind::OpenAi | ProviderKind::Xai => None,
        };
        Ok(Self {
            runtime: tokio::runtime::Runtime::new()?,
            client: ModelProviderClient::from_model_with_default_auth(&model, default_auth)?,
            model,
            enable_tools,
            emit_output,
            allowed_tools,
...
impl ApiClient for DefaultRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        self.runtime.block_on(async {
            ...
            let mut think_filter = ThinkBlockFilter::default();
        ...
        loop {
            if self.inside_block {
                ...
                if flush_all {
                    ...
                    let keep = THINK_CLOSE.len().saturating_sub(1).min(self.buffer.len());
                    let drain_len = safe_prefix_len(&self.buffer, self.buffer.len().saturating_sub(keep));
                    self.buffer.drain(..drain_len);
                }
                break;
            }

            if let Some(start) = self.buffer.find(THINK_OPEN) {
            ...
            if flush_all {
                ...
                let keep = THINK_OPEN.len().saturating_sub(1).min(self.buffer.len());
                let emit_len = safe_prefix_len(&self.buffer, self.buffer.len().saturating_sub(keep));
...
fn safe_prefix_len(value: &str, max_bytes: usize) -> usize {
...
fn strip_think_blocks(text: &str) -> String {
    let mut filter = ThinkBlockFilter::default();
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    if first_line.is_empty() {
        format!("\x1b[38;5;110m•\x1b[0m \x1b[1m{name}\x1b[0m")
    } else {
        format!("\x1b[38;5;110m•\x1b[0m \x1b[1m{name}\x1b[0m {first_line}{rest}")
    }
}
...
fn format_read_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    format!(
        "{icon} \x1b[38;5;245mread_file\x1b[0m {path} \x1b[2m(lines {}-{} of {})\x1b[0m\n{}",
        start_line,
...
fn format_write_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    let line_count = parsed
        ...
        .map_or(0, |content| content.lines().count());
    format!(
        "{icon} \x1b[38;5;245mwrite_file\x1b[0m {} {path} \x1b[2m({line_count} lines)\x1b[0m",
...
fn format_edit_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    match preview {
        Some(preview) => format!("{icon} \x1b[38;5;245medit_file\x1b[0m {path}{suffix}\n{preview}"),
        None => format!("{icon} \x1b[38;5;245medit_file\x1b[0m {path}{suffix}"),
    }
}
...
fn format_glob_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    if filenames.is_empty() {
        format!("{icon} \x1b[38;5;245mglob_search\x1b[0m matched {num_files} paths")
    } else {
        format!("{icon} \x1b[38;5;245mglob_search\x1b[0m matched {num_files} paths\n{filenames}")
    }
}
...
fn push_output_block(
    ...
    match block {
        OutputContentBlock::Text { text } => {
            ...
            if !visible.is_empty() {
                let rendered = TerminalRenderer::new().markdown_to_ansi(&visible);
                write!(out, "{rendered}")
                    .and_then(|()| out.flush())
                    .map_err(|error| RuntimeError::new(error.to_string()))?;
                events.push(AssistantEvent::TextDelta(visible));
...
mod tests {
    use super::{
        ...
        strip_think_blocks, CliAction, CliOutputFormat, InternalPromptProgressEvent,
        InternalPromptProgressState, SlashCommand, StatusUsage, ThinkBlockFilter, DEFAULT_MODEL,
    };
 ...
 M src/main.rs",
    ...
    fn tool_rendering_helpers_compact_output() {
        ...
        assert!(done.contains("src/main.rs"));
    ...
    #[test]
    fn strip_think_blocks_removes_embedded_reasoning() {
        assert_eq!(
    ...
    #[test]
    fn think_block_filter_handles_streamed_chunks() {
        let mut filter = ThinkBlockFilter::default();
        assert_eq!(filter.push("<thi"), "");
        assert_eq!(filter.push("nk>hidden"), "");
        assert_eq!(filter.push("</think>Visible"), "V");
        assert_eq!(filter.flush(), "isible");
    }

    #[test]
    fn think_block_filter_preserves_multibyte_boundaries() {
        let mut filter = ThinkBlockFilter::default();
        assert_eq!(filter.push("对 "), "");
        assert_eq!(filter.push("**C"), "");
        assert_eq!(filter.flush(), "对 **C");
    }

    #[test]
    fn push_output_block_skips_think_text() {
        let mut out = Vec::new();
        let mut events = Vec::new();
        let mut pending_tool = None;
        ...
        assert_eq!(events, vec![AssistantEvent::TextDelta("Answer".to_string())]);
    }
